...
const (
	...
	PLUGIN_RUNTIME_PROCESS Runtime = "PROCESS"
)

func IsSupportedRuntime(runtime string) bool {
	runtimeUpper := strings.ToUpper(runtime)
	return runtimeUpper == string(PLUGIN_RUNTIME_PYTHON) || runtimeUpper == string(PLUGIN_RUNTIME_NODEJS) || runtimeUpper == string(PLUGIN_RUNTIME_GO) || runtimeUpper == string(PLUGIN_RUNTIME_SCRIPT) || runtimeUpper == string(PLUGIN_RUNTIME_PROCESS)
}
